...
(function () {
  ...
  function markAutoRenewRedirect(tagId, renewUrl) {
  ...
  function isSameRenewRef(storedUrl, referrer) {
    if (!storedUrl || !referrer) return false;
    try {
      const stored = new URL(storedUrl, location.href);
      const ref = new URL(referrer, location.href);
      if (stored.protocol !== ref.protocol) return false;
      if ((stored.hostname || '') !== (ref.hostname || '')) return false;
      if ((stored.port || '') !== (ref.port || '')) return false;
      const normalizePath = (path) => (path || '').replace(/\/+$/, '');
      if (normalizePath(stored.pathname) !== normalizePath(ref.pathname)) return false;
      return true;
    } catch {
      return false;
    }
  ...
  function consumeAutoRenewRedirect(tagId) {
    ...
    if (memory.renewUrl && !isSameRenewRef(memory.renewUrl, document.referrer)) {
      return false;
    }
    clearAutoRenewMemory();
    return true;
