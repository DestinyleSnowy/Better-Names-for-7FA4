...
- `worker` 平台能力收敛
- `feature` 独立注册与按路由筛选
- `feature flag` 默认值与 migration 骨架

下一步的重点不再是继续铺目录，而是开始把旧功能按 feature 迁入，并逐步删掉过渡遗留目录 `src/content`、`src/background`、`src/panel`、`src/popup`。
