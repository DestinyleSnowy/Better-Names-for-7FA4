...
项目现在包含 Firefox 所需的 MV3 后台脚本回退配置，但请注意：

- 需要较新的 Firefox（建议 `121+`）。
- Release 里的压缩包本质上仍是源码包；在 Firefox 中测试时，请先解压，然后打开 `about:debugging#/runtime/this-firefox`，选择“临时载入附加组件”，并选中 `Better-Names-for-7FA4/manifest.json`。
- 如果需要长期安装，而不是临时调试，仍然需要带 Gecko ID 的已签名包。
...
Better Names for 7FA4 核心功能已开发完成并已趋于稳定，目前处于低频维护状态。由于维护者精力有限，如果您想成为新的维护者或核心开发者，可以向我们联系。
