console.log("[BG] Background.js正在加载")
      ...
      `;
            ...
            try {
                ...
                function buildHeaderMapping(cells) {
                    ...
                    for (const el of cells) {
                        ...
                        if (/排序|排名/.test(text) || /rank/i.test(id)) {
                            mapping.push({kind: 'rank'});
                            continue;
                        }
                        if (/昵称|名字|用户名/.test(text) || /nickname/i.test(id)) {
                            mapping.push({kind: 'name'});
                            continue;
                        }
                        if (/学校/.test(text)) {
                            mapping.push({kind: 'school'});
                            continue;
                        }
                        if (/年|时年|年级/.test(text)) {
                            mapping.push({kind: 'grade'});
                            continue;
                        }
                        if (/总分/.test(text) || id.indexOf('sum') !== -1) {
                            sumSeen++;
                            mapping.push({kind: 'sum', part: sumSeen === 1 ? 'c' : 'r'});
                            continue;
                        }
                ...
                if (window.jQuery && typeof window.jQuery.get === 'function') {
                    ...
                    for (let p = startPage; p < startPage + MAX_HTML_PAGES && missingSet.size > 0; p++) {
                        try {
                            const q = Object.assign({}, paramsObj);
                            q.page = p;
                            console.log('show-all: fetching table html page', p, 'for table_id=', table_id);
                            const html = await new Promise((resolve, reject) => {
                                try {
                                    window.jQuery.get(`/progress/${type}_table/html`, q, res => resolve(res));
                                } catch (e) {
                                    reject(e);
                                }
                            });
        ...
        async function gatherAllPages(originalUrlStr, opts = {}) {
            ...
            const emit = (payload) => {
                ...
                try {
                    progressCb(Object.assign({}, base, data));
            ...
            while (true) {
                if (page > pageLimit) {
                    console.warn('show-all: page limit reached', page);
                    emit({text: `达到抓取页数上限 ${pageLimit}，停止合并`, stage: 'limit', tone: 'warning'});
                    break;
                }
                if (totalHint && mergedUsers.length >= totalHint) {
                    console.log('show-all: reached totalHint', totalHint);
                    break;
                }
                ...
                try {
                    ...
                    if (j && j.success === false) {
                        console.warn('show-all: page', page, 'returned success=false - stop');
                        emit({
                            text: `第 ${page} 页返回 success=false，停止合并`,
                            stage: 'error',
                            tone: 'warning',
                            currentPage: page
                        });
                    ...
                    if (!a || a.length === 0) {
                        console.log('show-all: page', page, 'empty - stop');
                        emit({
                            text: `第 ${page} 页没有更多数据，停止合并`,
                            stage: 'empty',
                            tone: 'info',
                            currentPage: page
                        });
                    ...
                    emit({
                        text: `第 ${page} 页抓取完成，累计 ${mergedUsers.length} 条数据`,
                        stage: 'page-complete',
                        currentPage: page
                    });
                } catch (e) {
                    ...
                    emit({
                        text: `抓取第 ${page} 页失败：${e && e.message ? e.message : e}`,
                        stage: 'error',
                        tone: 'error',
                        currentPage: page
                    });
...
// waitForjQuery & patch $.get (manual trigger UI)
        function waitForjQuery(timeout = 6000) {
            return new Promise((resolve, reject) => {
                ...
                const iv = setInterval(() => {
                    ...
                    if (t >= timeout) {
                        clearInterval(iv);
                        return reject(new Error('jQuery not found'));
                    }
                }, 100);
            });
        }
        ...
        (async () => {
            try {
                const $ = await waitForjQuery(6000).catch(e => {
                    console.warn('show-all: jQuery not found in time');
                    return null;
                });
                if (!$ || typeof $.get !== 'function') {
                    console.warn('show-all: cannot patch $.get');
                    return;
                }
                ...
                function startMergeDrag(event, container) {
                    ...
                    const rect = container.getBoundingClientRect ? container.getBoundingClientRect() : {
                        top: DEFAULT_CONTAINER_TOP,
                        left: DEFAULT_CONTAINER_RIGHT
                    };
                    mergeDragState = {
                    ...
                    if (typeof container.setPointerCapture === 'function' && mergeDragState.pointerId !== null) {
                        try {
                            container.setPointerCapture(mergeDragState.pointerId);
                        } catch (_) { /* ignore */
                        }
                    }
                ...
                function finishMergeDrag(event) {
                    ...
                    if (container) {
                        ...
                        if (mergeDragState.pointerId !== null && typeof container.releasePointerCapture === 'function') {
                            try {
                                container.releasePointerCapture(mergeDragState.pointerId);
                            } catch (_) { /* ignore */
                            }
                        }
                    }
...
function matchesPatternToRegExp(pattern) {
    const scheme = pattern.split('://')[0];
    let host = pattern.split('://')[1].split('/')[0];
    let port = host.split(':')[1] || "";
    if (port) {
        host = host.split(':')[0];
        port = ":" + port;
    }
    const path = "/" + pattern.split('/')[3] || "";
    const schemeRegex = scheme === '*' ? '[^:]+://' : scheme + '://';
    const hostRegex = host.replaceAll(".", "\\.").replaceAll("*", '[^:/.]*');
    const portRegex = port.replaceAll("*", '[^/]*');
    const pathRegex = path.replace(/[+\-.\\\[\]?]/g, '\\$&').replaceAll("*", '[^?#]*');
    const all = schemeRegex + hostRegex + portRegex + pathRegex;
    return new RegExp(all);
}

/**
 * 判断 URL 是否与 matches 模式匹配
 * @param {string} pattern - matches 模式
 * @param {string} url - 要测试的完整 URL
 * @returns {boolean} 是否匹配
 */
function matchesPattern(pattern, url) {
    try {
        const regex = matchesPatternToRegExp(pattern);
...
// 添加Content-Scripts
function addContentScripts(JS_FILES, CSS_FILES, MATCH) {
    function isTargetHost(hostname) {
        for (let host of MATCH) {
            if (matchesPattern(host, hostname))
                return true;
        }
        return false;
    }
    ...
    browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
        if (changeInfo.status === 'complete') {
            if (isTargetHost(tab.url)) {
                try {
                    console.log(`[BG] 目标页面: ${tab.url}（Id：${tab.id}），开始注入 CSS 和 JS`);
                    // 1. 先注入 CSS（确保样式先就位）
...
addContentScripts(
    [
        ...
        'content/gm-shim.js',
        'content/panel/panel.js',
        'content/main.js'
    ],
    ['content/panel/panel.css'], // 需要注入的 CSS
    [
        'http://*.7fa4.cn:8888/*',
        "http://*.7fa4.cn:5283/*",
        "http://10.210.57.10:8888/*",
        "http://10.210.57.10:5283/*",
        "http://211.137.101.118:8888/*",
        "http://211.137.101.118:5283/*",
    ] // 匹配的 URL 模式
);
addContentScripts(
    [
        'browser-polyfill.min.js',
        "content/libs/marked.min.js",
        "content/libs/katex/katex.min.js",
        "content/libs/katex/auto-render.min.js",
        "content/render_logic.js"
    ],
    ["content/libs/katex/katex.min.css"],
    [
        "http://in.7fa4.cn:8888/user_plans/*",
        "http://in.7fa4.cn:5283/user_plans/*",
        "http://jx.7fa4.cn:8888/user_plans/*",
        "http://jx.7fa4.cn:5283/user_plans/*",
        "http://10.210.57.10:8888/user_plans/*",
        "http://10.210.57.10:5283/user_plans/*",
        "http://211.137.101.118:8888/user_plans/*",
        "http://211.137.101.118:5283/user_plans/*",
        "http://in.7fa4.cn:8888/review/user_tags/html?tag_id=*",
        "http://in.7fa4.cn:5283/review/user_tags/html?tag_id=*",
        "http://jx.7fa4.cn:8888/review/user_tags/html?tag_id=*",
        "http://jx.7fa4.cn:5283/review/user_tags/html?tag_id=*",
        "http://10.210.57.10:8888/review/user_tags/html?tag_id=*",
        "http://10.210.57.10:5283/review/user_tags/html?tag_id=*",
        "http://211.137.101.118:8888/review/user_tags/html?tag_id=*",
        "http://211.137.101.118:5283/review/user_tags/html?tag_id=*"
    ]
)
console.log("[BG] Background.js加载完毕")