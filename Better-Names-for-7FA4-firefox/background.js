        ...
        chrome.storage.local.get(['selectedSubmitter'], (items) => {
            try {
                ...
                if (!hasStoredValue && selectedSubmitter && selectedSubmitter !== 'none') {
                    try {
                        chrome.storage.local.set({selectedSubmitter});
                    } catch (_) { /* ignore */
                    }
                }
...
chrome.webRequest.onHeadersReceived.addListener(
    function (details) {
        ...
        console.log("headers of ", details.url, ":", headers);
        headers.push({
            name: "Access-Control-Allow-Origin",
            value: "*"
        });

        // 返回修改后的响应头，注意必须有 "responseHeaders" 属性
        return {responseHeaders: headers};
    },
    {
        urls: [
            "http://jx.7fa4.cn:8888/*",
            "http://in.7fa4.cn:8888/*",
            "http://jx.7fa4.cn:5283/*",
            "http://in.7fa4.cn:5283/*",
...
const CSS_FILES = ['content/panel/panel.css']; // 需要注入的 CSS

function isTargetHost(hostname) {
    return hostname.endsWith('7fa4.cn') ||
        hostname === '10.210.57.10' ||
        hostname === '211.137.101.118';
}
...
browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    if (changeInfo.status === 'complete' && tab.url) {
        try {
            ...
            if (isTargetHost(url.hostname) && TARGET_PORTS.includes(url.port)) {
                console.log(`[BG] 目标页面: ${tab.url}，开始注入 CSS 和 JS`);
                ...
                for (const file of CSS_FILES) {
                    try {
                        ...
                        console.log(`[BG] 已注入 CSS: ${file}`);
                    } catch (err) {
                        console.error(`[BG] 注入 CSS ${file} 失败:`, err);
                    }
                }
                ...
                for (const file of JS_FILES) {
                    try {
                        ...
                        console.log(`[BG] 已注入 JS: ${file}`);
                    } catch (err) {
                        console.error(`[BG] 注入 JS ${file} 失败:`, err);
                        break; // 停止后续注入
                    }
                }
            }
        } catch (e) {
            console.error('[BG] URL解析失败', e);
        }
    }
});
// background.js